...
//     displayOrders
//       .filter((o) => (filter === "all" ? true : o.paymentPeriod === filter))
//       .filter(
//         (o) =>
...
export default function CreditsPage() {
  ...
  // Fetch orders with Appwrite query
  const fetchOrders = async (period: string) => {
    try {
      let queries: any[] = [];
      if (period !== "all") {
        queries.push(Query.equal("paymentPeriod", period));
      }
  ...
  // Fetch order items (all, because needed for payments merge)
  const fetchOrderItems = async () => {
    try {
      const res = await databases.listDocuments(
        DATABASE_ID,
  ...
  useEffect(() => {
    fetchOrders(filter); // re-fetch when filter changes
    fetchOrderItems();
  }, [filter]);
  ...
  const handlePaymentSubmit = async () => {
    ...
    if (existingItem) {
      const existingPayments = existingItem.payments
        ? JSON.parse(existingItem.payments)
        : [];
      const updated = {
        ...existingItem,
        receivedAmount:
          Number(existingItem.receivedAmount) + paymentAmount,
        balanceAmount:
          Number(existingItem.totalAmount) -
          (Number(existingItem.receivedAmount) + paymentAmount),
        payments: [
          ...existingPayments,
          { amount: paymentAmount, mode: paymentMode, receivedDate },
        ],
      };
      ...
      await databases.updateDocument(
        ...
        {
          ...updated,
          payments: JSON.stringify(updated.payments),
          mode: paymentMode,
        }
      );
    } else {
  ...
  const generatePDF = (user: any) => {
    ...
    user.orders.forEach((order: any, idx: number) => {
      doc.text(
        `Order ${idx + 1} (ID: ${order.$id.slice(-6)} | Date: ${new Date(
          order.$createdAt
        ).toLocaleDateString()})`,
        10,
        y
  ...
  return (
    <div style={{ padding: 20, fontFamily: "Arial, sans-serif" }}>
      ...
      {mergedOrders.map((user) => (
        <div
          key={user.userId + "_" + user.paymentPeriod}
          style={{
            border: "1px solid #ddd",
            borderRadius: 10,
            padding: 20,
        ...
        >
          ...
          >
            <div>
              ...
              <p>{user.userPhone}</p>
              <p>
                Total: <b>{user.totalAmount}</b> | Received:{" "}
                <b>{user.receivedAmount}</b> | Balance:{" "}
                <b
                  style={{
                    color: user.balanceAmount === 0 ? "green" : "red",
                  }}
                >
                  {user.balanceAmount}
                </b>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {user.balanceAmount > 0 && (
                <button
                  onClick={() =>
                    openPaymentPopup(
                      user.orders.find((o: any) => o.balanceAmount > 0),
                      true
                    )
                  }
                  style={{
            ...
            {user.orders.map((order: any) => (
              <div
                ...
                style={{
                  ...
                  marginBottom: 8,
                  backgroundColor:
                    order.balanceAmount === 0 ? "#e6ffed" : "#fff9f0",
                }}
              >
                <div
                  style={{
                ...
                >
                  <div>
                    ...
                    <p style={{ margin: 0 }}>
                      Total: {order.totalAmount} | Received:{" "}
                      {order.receivedAmount} | Pending: {order.balanceAmount}
                    </p>
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    {order.balanceAmount > 0 && (
                    ...
                    <tbody>
                      {order.payments.map((p: any, idx: number) => (
                        ...
                        >
                          ...
                          <td style={{ padding: 8 }}>{p.mode}</td>
                          <td style={{ padding: 8 }}>
                            {new Date(p.receivedDate).toLocaleString()}
                          </td>
                        </tr>
                      ))}
                    </tbody>
